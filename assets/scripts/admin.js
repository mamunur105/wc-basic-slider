...
(function ($) {
  ...
  $(document).ready(function () {
     ...
     *
     */
    $('.bsfw-mlh-notice').on('click', '.notice-dismiss, .bsfw-mlh-notice-action', function () {
      var $this = $(this);
      var admin_ajax = bsfw_mlh_script.admin_ajax;
      var parents = $(this).parents('.bsfw-mlh-notice');
      ...
      var data = {
        ...
        cx_nonce: bsfw_mlh_script.ajx_nonce
      };
      jQuery.ajax({
        type: 'POST',
        url: admin_ajax,
        ...
        success: function success(response) {
          if (response) {
            $this.parents('.bsfw-mlh-notice').remove();
          }
        }
      });
    });
  ...
  *************************************************************/
  ...
  if ($pSlider.elExists()) {
    ...
    var swiperOptions = {
      loop: true,
      speed: 1000,
      effect: navigator.userAgent.toLowerCase().indexOf('firefox') > -1 ? 'fade' : 'slide',
      // Show fade effect instead of parallax slide in Firefox
