...
class Dependencies {
	...
	const PLUGIN_NAME = 'Category slider for WooCommerce';
	...
	public function check() {
		...
		add_action( 'wp_ajax_bsfw_plugin_activation', [ __CLASS__, 'activate_plugin'] );
                ...
                (function ($) {
                    ...
                    function ajaxActive( that, plugin ){
                        ...
                        $.ajax({
                            ...
                            data: {
                                action: 'bsfw_plugin_activation',
                                plugin_slug: plugin.slug ? plugin.slug : null,
                                activation_file: plugin.file_name,
                                bsfw_wpnonce: '<?php echo wp_create_nonce( Helpers::$nonceId ); ?>',
                            },
                            type: 'POST',
                            beforeSend() {
                                that.html( 'Activation Prosses Running... <div class="bsfw-loader"></div>' );
                            },
                            success(response) {
                            ...
                            .on('click', function (e) {
                                ...
                                if ( plugin.slug ) {
                                    wp.updates.installPlugin({
                                        ...
                                        installing: function () {
                                            that.html( 'Installing plugin... <div class="bsfw-loader"></div>' );
